...
def processMap = { def map, def geoList ->
    def keys = map.keySet().sort() { a, b ->
        def indexA = geoList.indexOf(a)
        def indexB = geoList.indexOf(b)

        if (indexA == -1) {
            System.err.println "TRACER problem with ${a}"
        }
        if (indexB == -1) { 
            System.err.println "TRACER problem with ${b}"
        }
...
def readGeoList = { def geoFile ->
    def list = []

    def regex = /"(.+)",".+",.+,\-.*/

    new File(geoFile).eachLine { line ->
...
def geoList = readGeoList(geoFile)

processMap(map, geoList)
